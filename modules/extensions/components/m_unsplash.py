...
unsplash_link = "https://unsplash.com/"\
                "?utm_source=iKetan-dct&utm_medium=referral"
...
class Image:
    def __init__(
            self, _id: str, desc: str, author: Author, color: Color,
            link: Link, tags: list, dimension: Dimension, likes: int
            ):
        self._id = _id
...
def find_image(keyword="", content_filter="high", _random=False):
    ...
    if not _random:
        ...
        position = random.randint(1, 10) if content["total"] >= limit \
            else random.randint(1, content["total"])
