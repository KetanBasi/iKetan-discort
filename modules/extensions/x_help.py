...
from main import botDescription, botName, botOwner, botVersion, prefix, adminPrefix
...
class Help(commands.Cog):
    ...
    async def help(self,
        ...
        if not input:           # ? Means "if the 'input' variable has no member"
            ...
            if uncategorized:
                embed.add_field(name='Uncategorized commands',
            ...
            for cog in self.client.cogs:
                ...
                if cog.lower() == input[0].lower():
                    ...
                    commandFound = False
                    for command in self.client.get_cog(cog).get_commands():
                        if (not command.hidden) and (command.name.lower() == input[0].lower()):
                            command_help = command.help
                            try:
                                command_help = command_help.format(prefix, adminPrefix)
                            except AttributeError:
                ...
                embed = discord.Embed(title='Unknown category / command',
